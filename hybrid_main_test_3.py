...
def evaluate_save_to_file (model, test_iterator):
  ...
  for batch in test_iterator:
    attention_predictions = attention_model(batch.text, batch_size=len(batch)).squeeze(1)
    cnn_predictions = cnn_model(batch.text).squeeze(1)
...
models = [cnn_model, attention_model]
